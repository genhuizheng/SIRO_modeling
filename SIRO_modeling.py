...
#calculate the seasonal specific lambda
def calculate_seasonal_specific_lambda(Time, input_lambda, spring_fall_rate, summer_winter_rate):
    if Time % 365 <= 90 or Time % 365 >= 365 - 91:
        return input_lambda * spring_fall_rate
    else:
        return input_lambda / summer_winter_rate
...
def run_one_model(input_overall_time, input_lambda, n0, n1, n2, n3, season_rate):
    ...
    for T_current in range(0, overall_time):
        seasonal_lambda = calculate_seasonal_specific_lambda(T_current+1, input_lambda, spring_fall_rate, summer_winter_rate)
        lambda_1_m = seasonal_lambda / n0
        #the probability that every infected person infects the suspectible person
        lambda_1_n = seasonal_lambda
        #the probability that every infected person infects the person recovering from infection (recovery)
        """range from the 2nd to the 4th"""
        lambda_list_from_2_to_4 = [seasonal_lambda * n1, seasonal_lambda * n2, seasonal_lambda * n3]
...
R0 = (1.2163 + 1.6135)/2
season_rate = 1.02
data_list = run_one_model(6000, 2.51e-05, 2, 0.7, 0.5, 0.3, season_rate)
to_text(data_list, 'output_middle_{0}_SI_1.xlsx'.format(int(m)))

R0 = 1.2163
season_rate = 1.04
data_list = run_one_model(6000, 3.86e-05, 2, 0.7, 0.5, 0.3, season_rate)
to_text(data_list, 'output_upper_{0}_SI_1.xlsx'.format(int(m)))


R0 = 1.6135
season_rate = 1.004
data_list = run_one_model(6000, 1.34e-05, 8, 0.7, 0.5, 0.3, season_rate)
to_text(data_list, 'output_lower_{0}_SI_1.xlsx'.format(int(m)))